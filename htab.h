...
//###############################################################################
// vytvorena struktura pro prvky
typedef struct htab_item
{
    htab_pair_t *pair;           // Klic a hodnota
    struct htab_item *next;      // Ukazatel na dalsi prvek(polozku)
}htab_list_t;

// vytvorena struktura pro tabulku
struct htab
{
    size_t actual_size;         // Pocet zaznamu(aktualni)           // aktuální počet záznamů [(key,data),next]
    size_t n_size_arr;          // Velikost array (pocet polozek)    // velikost pole ukazatelů (počet položek)
    htab_list_t *htab_ptr[];    // Array ukazatelu na prvky(polozky) //  ukazatel na dynamicky alokované pole ukazatelů
};
